$(function () {
  ...
  var draft = {
    ...
    clickEvent: function () {
      ...
      $('#delAll').on('click', function () {
        if ($(this).text() == '批量操作') {
          ...
          $(this).text('批量操作').prev().addClass('show');
    ...
    // 编辑和删除操作
    editAndDelEvent: function () {
      var that = this;
      $('#table_data').off('click').on('click', 'a,input', function () {
        var dataId = $(this).data('id');
        var dataNid = $(this).data('n_id');
        ...
        if (dataId == 1) {       // 编辑
          ...
          if (confirm('您确认删除吗？')) {
            $.ajax({
              url: ajaxUrl + 'mine/out',
              type: 'get',
              data: {
                n_id: dataNid
              },
              success: function (res) {
                // console.log('删除成功数据',res);
                if (res.status == 1000) {
                  that.getInitData();
                }
              },
